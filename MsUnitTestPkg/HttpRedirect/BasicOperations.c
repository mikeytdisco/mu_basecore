...
This module contains multiple code fragment used by DFCI for Http requests.

There are some WORKAROUND functions used to test an alternate path that sends
each http request through a different Http Child instead of the expected just issuing
multiple requests.
...
#define HEADER_AGENT_VALUE   "Test-Agent"
 ...
 *
 * If the parameter NetworkRequest is NULL, only timeout - no event, no poll
 *
...
EFI_STATUS
EventWait (
    IN  NETWORK_REQUEST  *NetworkRequest  OPTIONAL,
...
EFIAPI TimerTick (
    ...
    NetworkRequest = (NETWORK_REQUEST *) Context;
...
ConfigureHTTP (
    ...
    if (NetworkRequest->Http.ConfigData.LocalAddressIsIPv6) {
        ...
        if (EFI_ERROR(Status) || (Address.Address.Addr[0] == 0)) {
            DEBUG((DEBUG_ERROR, "Configuring DHCP for Test. Code=%r\n", Status));
...
IssueRequest (
    ...
    DEBUG((DEBUG_INFO, "IssueRequest status = %r\n", Status));
...
#define CLEANUP_ALL       (CLEANUP_REQUEST | CLEANUP_RESPONSE | CLEANUP_STATUS | CLEANUP_NIC)
...
ProcessHttpRequest (
    ...
    Status = ConvertToCHAR16(Url, AsciiStrLen(Url), &RequestData.Url, NULL);
    if (EFI_ERROR(Status)) {
        return Status;
    }

    // Send request without the terminating NULL.
    if ( 0 != NetworkRequest->HttpRequest.BodySize ) {
    ...
    Status = BuildRequestHeaders (Url,
    ...
    Status = IssueRequest (NetworkRequest, &RequestToken);
    if (EFI_ERROR(Status)) {
        goto S_EXIT1;
    }
    ...
    Status = GetResponse (NetworkRequest, &ResponseToken);
    if (EFI_ERROR(Status)) {
        goto S_EXIT1;
    }
...
STATIC
EFI_STATUS
ProcessHttpRequestWithRetiesWORKAROUND (
    IN  NETWORK_REQUEST   *NetworkRequest,
    IN  EFI_HTTP_METHOD    HttpMethod,
    IN  CHAR8             *Url,
    IN  BOOLEAN            RetryOn202
...
ProcessAsyncRequest (
    ...
    if (NetworkRequest->HttpStatus.HttpStatus == HTTP_STATUS_302_FOUND) {
    	// Treat Temporary Relocation as an ACCEPTED with LOCATION:
    } else {
	    //
...
ProcessAsyncRequestWORKAROUND (
    ...
    Status = ProcessHttpRequestWithRetiesWORKAROUND (NetworkRequest,
                                           HttpMethod,
                                           Url,
                                           FALSE);
    if (EFI_ERROR(Status)) {
...
MainLogic (
    ...
    *DoneProcessing = FALSE;
    ...
    //
    // Step 1. Ping server with URL provided
    //

    Status = ProcessAsyncRequest (NetworkRequest,
                                  NetworkRequest->HttpRequest.Url,
                                  NetworkRequest->HttpRequest.HttpMethod,
                                  DoneProcessing);
    ...
    if (!EFI_ERROR(Status)) {
        ...
        //
        Status = UpdateFromJson (NetworkRequest->HttpResponse.Body,
    ...
    switch (Status) {
        ...
        case EFI_MEDIA_CHANGED:
            //
            //  EFI_MEDIA_CHANGED indicates that  Update Variables were set.  Exit to
...
MainLogicWORKAROUND (
    ...
    *DoneProcessing = FALSE;
    ...
    Status = ProcessAsyncRequestWORKAROUND (NetworkRequest,
                                            NetworkRequest->HttpRequest.Url,
                                            NetworkRequest->HttpRequest.HttpMethod,
                                            DoneProcessing);
...
TryEachNICThenProcessRequest (
    ...
    for (NicIndex = 0; (NicIndex < HandleCount) && !DoneProcessing; NicIndex++) {
        //
        // Clear the working section of NETWORK_REQUEST
        //
        ...
        //
        Status = MainLogic (NetworkRequest, &DoneProcessing);
        if (EFI_ERROR(Status)) {
...
/**
 * TryEachNICThenProcessRequestWORKAROUND
 *
 * @param[in]   NetworkRequest
 *
 * Returns      EFI_STATUS
...
TryEachNICThenProcessRequestWORKAROUND (
    ...
    for (NicIndex = 0; (NicIndex < HandleCount) && !DoneProcessing; NicIndex++) {
        ...
        // Process HTTP Recovery flow on this NIC. If it fails, check the next NIC
        //
        Status = MainLogicWORKAROUND (NetworkRequest, &DoneProcessing);
        if (EFI_ERROR(Status)) {
            DEBUG((DEBUG_INFO, "MainLogic error. Code=%r\n", Status));
        }
 ...
 * 2. Package the HTTPS certificate, and set the certificate variable
...
EnableCertificate (
  IN    NETWORK_REQUEST    *NetworkRequest
    ...
    if (NetworkRequest->HttpsCert == NULL) {
    	mOldCertificateList = NULL;
    	DEBUG((DEBUG_ERROR, "No certificate to install\n"));
    	return EFI_SUCCESS;
    }

    DEBUG((DEBUG_ERROR, "HttpsCert:\n"));
    DEBUG_BUFFER(DEBUG_INFO, NetworkRequest->HttpsCert, NetworkRequest->HttpsCertSize, DEBUG_DM_PRINT_ADDRESS | DEBUG_DM_PRINT_ASCII);
    ...
    Status = gRT->SetVariable(
                  ...
                  Cert);
    if (EFI_ERROR(Status)) {
        DEBUG((DEBUG_ERROR, "Unable to set Https certificate. Code=%r\n", Status));
        RestoreCertificates ();
    }
    return Status;
}
...
ProcessNetworkRequest (
    ...
    Status = EnableCertificate (NetworkRequest);
    if (EFI_ERROR(Status)) {
...
/**
 * Process Http Recovery using WORKAROUND algorithm
 *
 * @param NetworkRequest     - Private data
 * @param Message            - Where to store the status message
 *
...
ProcessNetworkRequestWORKAROUND (
    ...
    // Try every NIC in the system until one fills the first part of the request.
    Status = TryEachNICThenProcessRequestWORKAROUND (NetworkRequest);
